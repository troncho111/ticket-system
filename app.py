...
# Note: Email scheduler needs to run as separate process
# See EMAIL_SCHEDULER_SETUP.md for setup instructions
# GitHub Actions workflow is available at .github/workflows/email-scheduler.yml
...
with st.sidebar:
    ...
    report_type = st.selectbox(
        "בחר סוג דוח:",
        options=["📋 כרטיסים לרכישה", "💰 מכירות יומי", "📊 מכירות שבועי", "🔴 הזמנות לא שולמו"],
    ...
    elif report_type == "🔴 הזמנות לא שולמו":
        try:
            ...
            if unpaid_count > 0:
                ...
                st.info(f"🔴 {unpaid_count} הזמנות ממתינות לתשלום | סה״כ: €{total_amount:,.2f}")
                
                if st.button("📤 שלח תזכורת", use_container_width=True, type="primary", key="send_unpaid_reminder_btn"):
                    if email_recipient and '@' in email_recipient:
                        with st.spinner("שולח מייל תזכורת..."):
                            # Send email using daily_reminder function but override recipient
                            try:
                                from daily_reminder import get_resend_credentials
                                import resend
                                
                                api_key, from_email = get_resend_credentials()
                                if not api_key or not from_email:
                                    st.error("❌ לא נמצאו פרטי Resend API")
                                else:
                                    resend.api_key = api_key
                                    
                                    # Build email body (same as daily_reminder.py)
                                    import pytz
                                    from datetime import datetime
                                    from daily_reminder import generate_mark_paid_token, get_app_base_url
                                    ...
                                    email_body += f"<p style='background: #dc2626; color: white; padding: 10px; border-radius: 5px; font-size: 18px;'><strong>{unpaid_count} הזמנות ממתינות לתשלום</strong></p>"
                                    email_body += "<hr>"
                                    
                                    total_amount = 0
                                    for idx, order in enumerate(unpaid_orders):
                                        order_num = order.get('Order number', '-')
                                        event_name = order.get('event name', '-')
                                        docket = order.get('docket number', order.get('docket', order.get('Docket', '-')))
                                        source = order.get('source', '-')
                                    ...
                                    email_body += "<hr><p style='color: #666;'>תזכורת יומית ממערכת ניהול הזמנות כרטיסים</p>"
                                    
                                    result = resend.Emails.send({
                                        "from": from_email,
                                        "to": [email_recipient],
                                        "subject": f"🔴 תזכורת יומית - {unpaid_count} הזמנות לא שולמו! (€{total_amount:,.2f})",
                                        "html": email_body
                                    })
                                    
                                    st.success(f"✅ מייל תזכורת נשלח בהצלחה ל-{email_recipient} ({unpaid_count} הזמנות)!")
                            except Exception as e:
                                st.error(f"❌ שגיאה בשליחת המייל: {e}")
                    else:
                        st.error("❌ כתובת מייל לא תקינה")
            else:
                st.success("✅ אין הזמנות שלא שולמו - הכל מעודכן!")
        except Exception as e:
...
with tab8:
    st.header("🔴 תזכורת - הזמנות לא שולמו")
    st.markdown("דף זה מציג את כל ההזמנות שלא שולמו עם אפשרות לסמן אותן כשולמו")
    ...
    try:
        ...
        def generate_mark_paid_token(order_num, row_index):
            """Generate a verification token for mark-as-paid links"""
            secret = os.environ.get('SESSION_SECRET')
            if not secret or len(secret) < 10:
                return None
            data = f"{order_num}:{row_index}:{secret}"
        ...
        if not unpaid_orders:
            ...
            <div style="background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%); 
                ...
                <h2 style="color: white; margin: 0;">{len(unpaid_orders)} הזמנות ממתינות לתשלום</h2>
            </div>
            """, unsafe_allow_html=True)
            ...
            for idx, order in enumerate(unpaid_orders):
                ...
                <div style="background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); 
                            padding: 20px; border-radius: 10px; margin: 15px 0; border-left: 5px solid #dc2626;">
                    <div style="background: #dc2626; color: white; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
                        <h3 style="margin: 0; color: white;">הזמנה #{idx+1} - לא שולם</h3>
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                        <div><strong>מספר הזמנה:</strong> {order_num}</div>
                        <div><strong>שם אירוע:</strong> {event_name}</div>
                        <div><strong>מספר דוקט:</strong> {docket}</div>
                        <div><strong>מקור:</strong> {source}</div>
                        <div><strong>מספר הזמנה ספק:</strong> {supp_order}</div>
                        <div><strong>תאריך אירוע:</strong> {event_date}</div>
                        <div><strong>כמות:</strong> {qty}</div>
                        <div><strong>מחיר מקורי לכרטיס:</strong> {price_sold}</div>
                        <div style="grid-column: 1 / -1;"><strong>סכום לגבייה:</strong> {total_display}</div>
                    </div>
                    <div style="background: #dc2626; height: 3px; margin: 15px 0; border-radius: 2px;"></div>
                </div>
                """, unsafe_allow_html=True)
                    ...
                    if token:
                        ...
                        try:
                            ...
                            if headers:
                                ...
                                protocol = 'https' if 'streamlit' in host.lower() else 'http'
                                app_url = f"{protocol}://{host}"
                            else:
                                app_url = st.get_option("server.baseUrlPath") or ""
                        except:
                            app_url = ""
                        ...
                        with col2:
                            if st.button(f"✅ סמן כשולם (Done!)", key=f"mark_paid_{order_num}_{row_index}", type="primary", use_container_width=True):
                        ...
                        st.warning("⚠️ לא ניתן ליצור קישור מאובטח - חסר SESSION_SECRET")
            ...
            if total_amount > 0:
                ...
                <div style="background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%); 
                    ...
                    <h2 style="color: white; margin: 0;">סה״כ סכום לגבייה: €{total_amount:,.2f}</h2>
                </div>
                """, unsafe_allow_html=True)
            
            # Send email button
            st.markdown("---")
            if st.button("📧 שלח מייל תזכורת", key="send_reminder_email", type="primary"):
                with st.spinner("שולח מייל..."):
                    try:
                        from daily_reminder import send_daily_reminder_email
                        success = send_daily_reminder_email(unpaid_orders)
                        if success:
                            st.success(f"✅ מייל נשלח בהצלחה ל-{len(unpaid_orders)} הזמנות!")
                        else:
                            st.error("❌ שגיאה בשליחת המייל")
                    except Exception as e:
                        st.error(f"שגיאה: {e}")
